-- ============================================
-- ALSHHAB Platform — Supabase SQL Setup (Fixed)
-- ✅ تم إصلاح: policies متعارضة في enrollments
-- ============================================
...
-- ============================================
-- ✅ FIX #7: Policies — enrollments (بدون تعارض)
-- دمجنا الـ policies المتعارضة في policy واحدة لكل عملية
-- ============================================
...
drop policy if exists "enroll_admin_insert"  on public.enrollments;
drop policy if exists "enroll_admin_update"  on public.enrollments;

-- SELECT: المستخدم يشوف تسجيلاته هو، والأدمن يشوف الكل
...
-- ✅ INSERT موحّد: المستخدم يسجّل نفسه، أو الأدمن يسجّل أي حد
...
-- ✅ UPDATE موحّد: المستخدم يعدّل تسجيله، أو الأدمن يعدّل أي تسجيل
...
drop policy if exists "certs_admin_insert"  on public.certificates;
...
drop policy if exists "payreq_admin_select" on public.payment_requests;
drop policy if exists "payreq_admin_update" on public.payment_requests;

-- ✅ دمجنا الـ select في policy واحدة
create policy "payreq_select" on public.payment_requests for select
  using (
    auth.uid() = user_id
    OR exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
  );
create policy "payreq_insert" on public.payment_requests for insert
...
begin
  ...
  values (new.id, 'مرحباً بك في IBDA3! 🎉', 'ابدأ رحلتك التعليمية الآن واكسب أول 10 XP');
