...
const Courses = {
  ...
  async getAll(forceRefresh = false) {
    ...
    this._fetchPromise = (async () => {
      ...
      const fallback = JSON.parse(localStorage.getItem('alshehab_courses') || '[]');
  ...
  async getById(id) {
    ...
    const cached = this._cache || JSON.parse(localStorage.getItem('alshehab_courses') || '[]');
  ...
  async isEnrolled(courseId) {
    ...
    const localEnrolled = JSON.parse(localStorage.getItem('alshehab_enrolled') || '[]');
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        if (!localEnrolled.includes(courseId)) {
          ...
          localStorage.setItem('alshehab_enrolled', JSON.stringify(localEnrolled));
        }
        return true;
      }
    } catch {}
...
const Notifs = {
  ...
  async getAll(forceRefresh = false) {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        this._cache = data; this._cacheTime = Date.now();
        localStorage.setItem('alshehab_notifs', JSON.stringify(data));
        return data;
      }
    } catch {}
    const fallback = JSON.parse(localStorage.getItem('alshehab_notifs') || '[]');
    this._cache = fallback; this._cacheTime = Date.now();
    return fallback;
  },
  ...
  async add(title, body = '', targetUserId = null) {
    ...
    try {
      ...
      localStorage.setItem('alshehab_notifs', JSON.stringify(notifs.slice(0, 30)));
    }
