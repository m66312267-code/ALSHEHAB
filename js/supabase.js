...
const sb = {
  ...
  async signIn(email, password) {
    ...
    // جيب البروفايل كامل في الخلفية
    if (data.user?.id) {
      this.select('profiles', `id=eq.${data.user.id}&select=grade,governorate,phone,parent_phone,role,xp`)
        .then(rows => {
          const p = rows?.[0]; if (!p) return;
          if (p.grade)       localStorage.setItem('ibda3_grade',       p.grade);
          if (p.governorate) localStorage.setItem('ibda3_governorate', p.governorate);
          if (p.xp != null)  localStorage.setItem('ibda3_xp',          p.xp);
        }).catch(() => {});
    }
    return data;
  },
  ...
  async sendPhoneOTP(phone) {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data?.msg || data?.error_description || 'فشل إرسال الكود');
    return data;
  },

  // التحقق من OTP وإنشاء جلسة (للتسجيل أو التحقق)
  async verifyPhoneOTP(phone, token) {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data?.msg || data?.error_description || 'الكود غلط أو انتهت صلاحيته');
    return data;
  },

  // تسجيل دخول برقم الموبايل: ابحث عن الإيميل من profiles ثم signIn
  async lookupEmailByPhone(phone) {
    const rows = await this.select('profiles', `phone=eq.${phone}&select=id,email`);
    if (!rows || rows.length === 0) throw new Error('الرقم ده مش مسجّل — تأكد أو أنشئ حساب');
    const email = rows[0]?.email;
    if (!email) throw new Error('مش لاقي الإيميل المرتبط بالرقم ده');
    return email;
  },
  ...
  async signOut() {
    ...
    localStorage.removeItem('ibda3_grade');
    localStorage.removeItem('ibda3_governorate');
...
// ===== AUTH GUARD =====
// ══════════════════════════════════════════════════════
//  MAINTENANCE CHECK — من Supabase على كل الصفحات
// ══════════════════════════════════════════════════════
async function checkMaintenance() {
  // الأدمن يعدي عادي دايماً
  const cached = JSON.parse(localStorage.getItem('ibda3_profile') || 'null');
  if (cached?.role === 'admin') return;

  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/platform_settings?select=maintenance,maint_msg,maint_end&limit=1`,
    ...
    // التحقق من الـ role من Supabase (مش بس الـ cache)
    const session = JSON.parse(localStorage.getItem('sb_session') || 'null');
    if (session?.user?.id) {
      try {
        ...
        if (pRows?.[0]?.role === 'admin') return; // أدمن — يعدي
      } catch {}
    }

    // مش أدمن — أريه شاشة الصيانة
    _showMaintScreen(s.maint_msg, s.maint_end);
  } catch {
    // لو فشل الاتصال — خليه يكمل (أحسن من إنك توقف الموقع بالغلط)
  }
}

function _showMaintScreen(msg, endTime) {
  // إزالة أي محتوى موجود وعرض شاشة الصيانة
  document.body.style.overflow = 'hidden';
  const el = document.createElement('div');
  el.id = 'globalMaintScreen';
  el.style.cssText = `
  ...
  if (endTime) {
    countdownHTML = `
      <div id="_maintCountdown" style="display:flex;gap:16px;justify-content:center;margin-bottom:28px;">
        <div style="background:#1a1f2e;border:1px solid #2d3748;border-radius:12px;padding:16px 22px;text-align:center;">
          <b id="_mH" style="font-size:30px;font-weight:900;color:#00d4aa;display:block;">00</b>
          <span style="font-size:11px;color:#6b7280;">ساعة</span>
        </div>
        <div style="background:#1a1f2e;border:1px solid #2d3748;border-radius:12px;padding:16px 22px;text-align:center;">
          <b id="_mM" style="font-size:30px;font-weight:900;color:#00d4aa;display:block;">00</b>
          <span style="font-size:11px;color:#6b7280;">دقيقة</span>
        </div>
        <div style="background:#1a1f2e;border:1px solid #2d3748;border-radius:12px;padding:16px 22px;text-align:center;">
          <b id="_mS" style="font-size:30px;font-weight:900;color:#00d4aa;display:block;">00</b>
          <span style="font-size:11px;color:#6b7280;">ثانية</span>
        </div>
      </div>`;
  }

  el.innerHTML = `
    <div style="font-size:72px;margin-bottom:20px;animation:spin 4s linear infinite;">🔧</div>
    <h2 style="font-size:28px;font-weight:900;color:#fff;margin-bottom:10px;">قيد الصيانة</h2>
    <p style="color:#6b7280;font-size:14px;max-width:380px;line-height:1.8;margin-bottom:26px;">
      ${msg || 'المنصة في وضع الصيانة مؤقتاً. نعمل على تحسينها وستعود قريباً!'}
    </p>
    ${countdownHTML}
    <div style="background:rgba(0,212,170,0.08);border:1px solid rgba(0,212,170,0.2);border-radius:14px;padding:14px 24px;font-size:13px;color:#6b7280;">
      للتواصل: <a href="https://wa.me/201063786533" target="_blank" style="color:#00d4aa;font-weight:700;text-decoration:none;">واتساب 📞 01063786533</a>
    </div>
  ...
  if (endTime) {
    ...
    const timer = setInterval(() => {
      ...
      if (diff <= 0) { clearInterval(timer); document.getElementById('_maintCountdown')?.remove(); return; }
      const h = Math.floor(diff / 3600000);
      const m = Math.floor((diff % 3600000) / 60000);
      const s = Math.floor((diff % 60000) / 1000);
      const hEl = document.getElementById('_mH');
      const mEl = document.getElementById('_mM');
      const sEl = document.getElementById('_mS');
...
function requireAuth() {
  ...
  // تحقق من الصيانة تلقائياً مع كل requireAuth
  checkMaintenance();
  return true;
}
